...
# spell.word_frequency.load_words(['MSA', 'PRO-X/L', 'Peltor', 'Sordin', 'F-1',
#                                  'MK2', 'hand-held', 'medikit', 'AT-2', 'Salewa', 'BP',
#                                  'AP', 'mm', 'TT', '"Hawk"', '"Eagle"', '"Kite"', 'Wartech',
#                                  'MK3', '(black]', 'Blackhawk!', 'BlackRock', "Izhmash", 
#                                  "compatibles", 'HK', "A1", "MP7", "D3CRX"])
    ...
    def run(self, lock):
        ...
        while not self.need_quit:
            ...
            if diff_num < 370: 
                ...
                search_area1 = ((width/2)-33, (height/2)+43, (width/2)+80, (height/2)+55)
                search_area2 = search_area1
            ...
            while not found:
                ...
                #         tempWord = word.strip("()[]")
                #         print("Tempword0: ", tempWord)
                #         newWord = "(" + spell.correction(tempWord) + ")"
                #     elif "(" in word and "]" in word or "[" in word and ")" in word:
                #         tempWord = word.strip("()[]")
                #         print("Tempword0: ", tempWord)
                #         newWord = "(" + spell.correction(tempWord) + ")"
                #     elif "(" in word or "[" in word:
                #         tempWord = word.strip("()[]")
                #         print("Tempword1: ", tempWord)
                #         newWord = "(" + spell.correction(tempWord)
                #     elif ")" in word or "]" in word:
                #         tempWord = word.strip("()[]")
                #         print("Tempword2: ", tempWord)
                #         newWord = spell.correction(tempWord) + ")"
                #     else:
                #         newWord = spell.correction(word)
                #     print(word, " to ", newWord)
                ...
                rep = {" ": "_", "$": "", "/": "_", r"[\/\\\n|_]*": "_", "muzzle": "muzzlebrake", "brake": "", "7.6239": "7.62x39",
                       ...
                       "^['^a-zA-Z_]*$": "%E2%80%98", "AT-2": "AI-2", "®": "", "§": "5", "__": "_", "___": "", "MS": "M3", "xX": "X",
                ...
                while tryCounter <= tryLimit:
                    try:
                        ...
                        if tryCounter == 1:
                            ...
                            URL = 'https://tarkov-market.com/item/'+corrected_text.capitalize()
                        tryCounter = tryCounter + 1
                        print("Retry: ", tryCounter, " ", URL)
                        continue
                ...
                try:
                    URL2 = 'https://escapefromtarkov.gamepedia.com/'+corrected_text
            ...
            itemLastLowSoldPrice = soup1.find("div", {"class": "price last"}).text
            item24hrAvgPrice = soup1.findAll("div", {"class": "price-row"})[0].findChildren()[0].get_text()
            traderName = soup1.findAll("div", {"class": "desc"})[2].parent.find("div", {"class": "title"}).find("a").get_text()
            itemTraderPrice = soup1.findAll("div", {"class": "desc"})[2].parent.find("div", {"class": "price"}).get_text()
            questsListText = []
            quests = ""
            questchecker = soup2.findAll("span", {"id": "Quests"})
            if len(questchecker) == 1:
                lists = soup2.find("div", {"class": "mw-parser-output"}).findAll("ul")
                for child in lists:
